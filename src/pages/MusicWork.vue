<template>
  <div class="text-3xl p-2 border-b">音乐部门-作品范围编辑</div>
  <div class="w-full space-y-5 p-2 md:w-1/2 md:m-auto">
    <div>{{ '共' + workMusicListTemp.length + '个作品' }}</div>
    <div>
      <div>说明：</div>
      <div>*音乐部门的作品范围定义为：包含首次出现的曲目的作品</div>
      <div>*作品命名以THBWiki的中文译名为准，但不包括作品名称的“～”符号及其后面的英文</div>
      <div>*最后一项“其他”请勿改动</div>
      <div>*请勿抬杠</div>
    </div>
    <div v-for="(item, index) in workMusicListTemp" :key="item.name" class="flex justify-between p-5 rounded shadow">
    ...
    >
      ...
      <div class="flex flex-col space-y-2">
        ...
        <div class="flex items-center py-1 px-2">
          ...
          <VoteSelect v-model:selected="workMusicKindTemp" :item-list="kindList" />
        </div>
      </div>
...
import { Work, workMusicList, SelectList, kindList } from '../lib/musicWork'
import VoteSelect from '../components/VoteSelect.vue'

const aciveIndex = ref(0)
const workMusicListTemp = ref<Work[]>(workMusicList)
const workMusicTemp = ref<Work>(JSON.parse(JSON.stringify(workMusicListTemp.value[0])))
const workMusicKindTemp = ref<SelectList>(
  kindList.find((item) => item.value === workMusicTemp.value.kind) || kindList[0]
)
watch(workMusicTemp, () => {
  workMusicKindTemp.value = kindList.find((item) => item.value === workMusicTemp.value.kind) || kindList[0]
})
...
function edit(index: number): void {
  ...
  workMusicTemp.value = JSON.parse(JSON.stringify(workMusicListTemp.value[aciveIndex.value]))
  open.value = true
}
function confirmEdit(): void {
  workMusicListTemp.value[aciveIndex.value] = workMusicTemp.value
  workMusicListTemp.value[aciveIndex.value].kind = workMusicKindTemp.value.value
  close()
}
function close(): void {
  open.value = false
...
function deleteWork(): void {
  if (confirm('你确定要删除此作品吗？')) {
    workMusicListTemp.value.splice(aciveIndex.value, 1)
    alert('删除成功！')
  }
}

function outPutRes() {
  const json = workMusicListTemp.value
